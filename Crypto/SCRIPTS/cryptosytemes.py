...
class Chaine:
    ...
    def to_texte(self) -> Texte:
        ...
        Cette méthode permet de transformer une trame supposée de caractères codés sur un octet 
        en la chaîne de caractères correpsondante.
        ...
        assert set(entiers) == {0,1}, "On ne peut transformer en texte que des chaînes de bits (0 ou 1)"
        reste: int = len(entiers) % 8 
        nb_octets: int = len(entiers) // 8
        comp: int = 8 - reste if reste > 0 else 0
        # on complète éventuellement par des 0 s'il manque des entiers pour 
        # faire des "paquets de 8" 
        entiers += [0]*comp
        # On fabrique une liste de chaines d'entiers de longueur 8 
        # du type ["10101100", "00011010",...]
        octets: List[str] = [
            "".join([str(b) for b in entiers[i*8:(i+1)*8]]) for i in range(nb_octets)]
        # On concatène alors tous les octets pour former une unique chaîne de caractères 
        # et on en fait une instance de la classe Texte
        return Texte("".join([chr(int(octet, 2)) for octet in octets]))
...
class ECB:
    ...
    def __decoupe_en_blocs(self, les_entiers: Chaine) -> List[Chaine]:
        ...
        bs: List[int] = les_entiers.entiers + [0]*(0 if reste == 0 else n - reste)
        return [Chaine(bs[i*n:(i+1)*n]) for i in range(len(bs)//n)]
    ...
    def __permute(self, les_entiers: Chaine) -> List[Chaine]:
        ...
        """
        return [Chaine(self.cle.permute(bloc.entiers)) for bloc in self.__decoupe_en_blocs(les_entiers)]
    ...
    def chiffre(self, les_entiers: Chaine) -> Chaine:
        ...
        HîRzÂj
        >>> inv = ~ecb
        >>> inv.chiffre(crypt).to_texte()
        ananas
        """
        return Chaine([entier for bloc in self.__permute(les_entiers) for entier in bloc.entiers])
...
def chiffre_image(cle: Perm, image: str) -> None:
    ecb = ECB(cle)
    pic = Photo(image)
...
# chiffre_image(Perm(taille=157), "./marmote.jpg")
print(chiffre_texte(Perm(images=[0, 1, 3, 4, 2]), 'aaaaaaaaaaaaaaa'))
