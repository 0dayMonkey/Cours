def czar(message, decalage):
    return ''.join(chr((ord(c) - 65 + decalage) % 26 + 65) if 'A' <= c <= 'Z' else c for c in message) 
...
class Perm:
    ...
    def permute(self, xs: List[int]) -> List[int]:
        return [xs[i] for i in reversed(self.permutation)]
    ...
    def inverse(self) -> 'Perm':
        ...
        inverse_perm = Perm(len(self.permutation))
        inverse_perm.permutation = inverse_permutation
        return inverse_perm
